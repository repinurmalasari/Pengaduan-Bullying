            ...
            <div class="card" style="border: none; border-radius: 16px; box-shadow: 0 4px 24px rgba(0, 0, 0, 0.08); overflow: hidden;">
                ...
                <div style="background: linear-gradient(135deg, #4A7AB5, #5B8BC5); padding: 32px 24px;">
                    <div class="d-flex align-items-center" style="gap: 16px;">
                        ...
                        <div>
                            <h2 style="margin: 0; font-weight: 600; color: white; font-size: 28px;">Tindak Lanjut Awal</h2>
                            <p style="margin: 8px 0 0 0; color: rgba(255,255,255,0.9); font-size: 14px;">Formulir ini digunakan catat setelah lanjut dari Sistem Pengaduan</p>
                        </div>
                    </div>
                </div>
                ...
                <div style="padding: 32px;">
                    ...
                    <!-- Form -->
                    <form method="POST" action="{{ route('tindak-lanjut-awal.store') }}">
                        @csrf
                        ...
                        <!-- Aksi Awal - Hanya tampilkan jika belum diproses -->
                        @if(!isset($tindakLanjutAwal) || $tindakLanjutAwal->status !== 'diproses')
                        <div class="mb-4">
                        ...
                        @else
                        {{-- Tampilkan informasi korban dan pelaku yang sudah dipilih (read-only) --}}
                        <div class="mb-4">
                            <label class="form-label fw-500" style="color: #1a1a1a; font-size: 14px; margin-bottom: 12px;">
                            ...
                            <div style="background: #f0f9ff; padding: 16px; border-radius: 8px; border-left: 4px solid #4B7EC4;">
                                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                                    <div>
                                        ...
                                        <p style="margin: 4px 0 0 0; font-size: 14px; color: #1a1a1a; font-weight: 600;">
                                            {{ $tindakLanjutAwal->korban?->name ?? 'Tidak ada' }}
                                        </p>
                                    </div>
                                    <div>
                                        <p style="margin: 0; font-size: 12px; color: #666; font-weight: 500;">Pelaku yang Dipanggil</p>
                                        <p style="margin: 4px 0 0 0; font-size: 14px; color: #1a1a1a; font-weight: 600;">
                                            {{ $tindakLanjutAwal->pelaku?->name ?? 'Tidak ada' }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            {{-- Hidden inputs to preserve the values --}}
                            <input type="hidden" name="panggil_korban_id" value="{{ $tindakLanjutAwal->panggil_korban_id }}">
                            <input type="hidden" name="panggil_pelaku_id" value="{{ $tindakLanjutAwal->panggil_pelaku_id }}">
                        </div>
                        @endif
                        ...
                        <div class="mb-4">
                            ...
                            <div style="display: flex; gap: 12px;">
                                @if(isset($tindakLanjutAwal) && $tindakLanjutAwal->status == 'diproses')
                                {{-- Jika sudah diproses, tampilkan tombol Selesai --}}
                                <label id="statusSelesaiBtn" class="status-btn-selesai" style="flex: 1; padding: 12px 20px; border-radius: 8px; font-size: 13px; font-weight: 600; border: 2px solid #10b981; background: #10b981; color: white; cursor: pointer; text-align: center; transition: all 0.2s;">
                                    <input type="radio" name="status" value="selesai" checked style="display: none;">
                                    <i class="fas fa-check-circle"></i> Selesai (Kasus Ringan)
                                </label>
                                <label id="statusBKBtn" class="status-btn-bk" style="flex: 1; padding: 12px 20px; border-radius: 8px; font-size: 13px; font-weight: 600; border: 2px solid #f59e0b; background: white; color: #f59e0b; cursor: pointer; text-align: center; transition: all 0.2s;">
                                    <input type="radio" name="status" value="direkomendasi_bk" style="display: none;">
                                    <i class="fas fa-arrow-right"></i> Rekomendasikan ke BK
                                </label>
                                @else
                                {{-- Jika belum ada atau baru, tampilkan tombol Proses --}}
...
$(document).ready(function() {
    // Status button logic - handle different states
    @if(isset($tindakLanjutAwal) && $tindakLanjutAwal->status == 'diproses')
    // Jika sudah diproses
    function setStatusBtn(selected) {
        if (selected === 'selesai') {
            $('#statusSelesaiBtn').css({background:'#10b981',color:'white'});
            $('#statusBKBtn').css({background:'white',color:'#6366f1'});
            $('#statusSelesaiBtn input').prop('checked',true);
            $('#statusBKBtn input').prop('checked',false);
        } else if (selected === 'direkomendasi_bk') {
            $('#statusBKBtn').css({background:'#6366f1',color:'white'});
            $('#statusSelesaiBtn').css({background:'white',color:'#10b981'});
            $('#statusBKBtn input').prop('checked',true);
            $('#statusSelesaiBtn input').prop('checked',false);
        }
    }
    // Initial state
    var initialStatus = $('input[name="status"]:checked').val() || 'selesai';
    setStatusBtn(initialStatus);
    $('#statusSelesaiBtn').on('click',function(){setStatusBtn('selesai');});
    $('#statusBKBtn').on('click',function(){setStatusBtn('direkomendasi_bk');});
    @else
    // Jika belum ada atau baru
    function setStatusBtn(selected) {
    ...
    // Initial state
    var initialStatus = $('input[name="status"]:checked').val() || 'diproses';
    setStatusBtn(initialStatus);
